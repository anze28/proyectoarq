...
{
	$.post("../ajax/cliente.php?op=4",{idcliente : idcliente}, function(data, status)
	{
		...
		$("#imagenmuestra").show();
		$("#imagenmuestra").attr("src","../file/clientes/"+data.personaimagen);
...
function desactivar(idcliente)
{
	swal.fire({
		...
		if (result.value) {
			$.post("../ajax/cliente.php?op=2", {idcliente : idcliente}, function(e){
				mensaje=e.split(":");
					if(mensaje[0]=="1"){  
						swal.fire(
							'Mensaje de Confirmación',
...
function activar(idcliente)
{
	swal.fire({
		...
		if (result.value) {
			$.post("../ajax/cliente.php?op=3", {idcliente : idcliente}, function(e){
                console.log(e);
				mensaje=e.split(":");
					if(mensaje[0]=="1"){  
						swal.fire(
